...
interface IOHLCVChartComponentProps {
  name: string
  type: string
  ohlcv: IOHLCV[]
  width: number
...
export const OHLCVChartComponent = (props: IOHLCVChartComponentProps) => {
  const { name, type, ohlcv, width, height, ratio, drawVolume = true } = props

  const sortedOHLCV = ohlcv.sort((a: IOHLCV, b: IOHLCV) => {
  ...
  const xScaleProvider = discontinuousTimeScaleProvider.inputDateAccessor((d: IOHLCV) => new Date(d.timestamp))
  ...
  return (
    <React.Fragment>
      <ChartCanvas
        ...
        xExtents={xExtents}
      >
        <Chart id={1} yExtents={[(d: IOHLCV) => [d.high, d.low]]} padding={{ top: 40, bottom: 20 }}>
          ...
          <EdgeIndicator
            ...
            yAccessor={(d: IOHLCV) => d.close}
            fill={(d: IOHLCV) => (d.close > d.open ? '#6BA583' : '#FF0000')}
          />
        ...
        {drawVolume && (
          ...
          >
            ...
            <BarSeries
              yAccessor={(d: IOHLCV) => d.volume}
              fill={(d: IOHLCV) => (d.close > d.open ? '#6BA583' : '#FF0000')}
            />

            <CurrentCoordinate yAccessor={(d: IOHLCV) => d.volume} fill='#9B0A47' />
